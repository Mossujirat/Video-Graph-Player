            ...
            if self.mw.isthreadActiveGraph and self.mw.isthreadActiveVideo and self.mw.autoData:
                if self.mw.frameVideo < self.mw.endframe - self.mw.videoUpdate and self.mw.frameGraphUpdate < self.mw.maxGraphX - self.mw.graphUpdate:
                    ...
                    if self.mw.matching == 0:
                        ...
                        self.mw.ui.startActionButton.setEnabled(True)
                        self.mw.ui.stopActionButton.setEnabled(True)
...
class MyApp(QMainWindow):
    def __init__(self):
        ...
        # set icon
        self.setWindowIcon(QtGui.QIcon('asset/Logo2.ico')) 
    ...
    def startButtonPressed(self):
        if self.isthreadActiveVideo and self.isthreadActiveGraph:
            ...
            # For main menu
            self.ui.stopActionButton.setEnabled(False)
    ...
    def startActionButtonPressed(self):
        ...
        config["StartAction-Video"] = str(datetime.timedelta(seconds=self.timeVideo))
        self.startActionTime = self.timeStamp[self.frameGraphUpdate]
        config["StartAction-Graph"] = str(self.startActionTime)
        config.write()
        self.ui.statusbar.showMessage("Save successfully - StartAction-Video = " +str(datetime.timedelta(seconds=self.timeVideo))+ 
            ", StartAction-Graph = " +str(self.timeGraph)+ " second") 
        self.successMessage("Save successfully - StartAction-Video = " +str(datetime.timedelta(seconds=self.timeVideo))+ 
            ", StartAction-Graph = " +str(self.timeGraph)+ " second")
    ...
    def stopActionButtonPressed(self):
        ...
        config = ConfigObj(filename, encoding='utf8')
        config.filename = filename
        config["StopAction-Video"] = str(datetime.timedelta(seconds=self.timeVideo))
        self.stopActionTime = self.timeStamp[self.frameGraphUpdate]
        config["StopAction-Graph"] = str(self.stopActionTime)
        config.write()
        self.ui.statusbar.showMessage("Save successfully - StopAction-Video = " +str(datetime.timedelta(seconds=self.timeVideo))+ 
            ", StopAction-Graph = " +str(self.timeGraph)+ " second") 
        self.successMessage("Save successfully - StopAction-Video = " +str(datetime.timedelta(seconds=self.timeVideo))+ 
            ", StopAction-Graph = " +str(self.timeGraph)+ " second")
